...
let pushOrders = async (data) => {
  ...
  arr.forEach((element) => {
    nwarr.push(element.orders);
  });
  weeklyOrderData = nwarr;
  return nwarr;
};
...
let pushRevenue = async (data) => {
  ...
  arr.forEach((element) => {
    nwarr.push(element.total);
  });
  weeklyRevenueData = nwarr;
...
function nextPage() {
  if (curPage * pageSize < bestSellersData.length) curPage++;
  createTable(bestSellersData);
}
