...
let customFetcher = async (url, config = {}) => {
  ...
  return response, data, (bestSellersData = data?.bestsellers);
};

function createTable(data) {
  // let table = document.getElementById("table");
  // let data = Object.keys(dashboardData);
  // generateTableHead(table, data);
  // generateTable(table, dashboardData);
  console.log(data);
  let table = document.getElementById("table");
  let li = `<tr><th>Product Name</th><th>Price</th><th>Units Sold</th><th>Revenue</th></tr>`;
  data.forEach((element) => {
    li += `<tr><td>${element.product_name}</td><td>${element.price}</td><td>${element.units_sold}</td><td>${element.revenue}</td></tr>`;
  });
